...
- Offered, not started: a `convention_defence` field on the cards (§6.1); a
  cheap filter for structurally untestable papers (the timeout paper has 1.6
  display equations per 10k characters and reports solver output, so a good
  gold error in it is still an unusable card); a model comparison over the
  frozen 12.
